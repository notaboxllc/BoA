...
**Parameter descriptions (best practice — author once in `Env.java`).** Chain `.setDescription("...")` after `.setMutableAtRuntime()` to attach a rich "what it does + likely impact" string to a `Parameter`. Convention: plain prose (no markup), state the default and the direction of impact, and note any caveat (e.g. "on `-gpu`, applies on restart"). `Parameter.getDescription()` is serialized into the `paramList` payload (`"description"` field, omitted when null) and surfaced verbatim by the viewer's info ("i") popovers — a single source of truth that stays in lockstep with the code. As new assays add tunable parameters, give each a `setDescription` so its panel is self-documenting. The contractility knobs (`myoColTol`, `myoMotorAlignWithFilTolerance`, `myoSpring`, `myosinStallForce`, `myosinBreakForce`, `kOff`, `xCatch`, `xSlip`, `alphaCatch`, `alphaSlip`) are the first examples.
...
**Contractility assay** (`-contractility [N]`, alias `-bmContractile`, or param-file `contractilityAssay:true`): unlike the deflection/LP benchmarks it KEEPS the minifilament + chamber. Optional integer arg = dimers per minifilament end (`Env.numMyoDimersEachEndOfMiniFil`, launch-time only). A live stats HUD (`#contractilityHud`, **lower-left** in the viewer) reads the per-frame `contractility` block — instantaneous/cumulative-avg/recent-EWMA bound heads and tension, peak tension, per-anchor tension, and first-bind step. The block (incl. the `stats` sub-object) is emitted by `ThreeJSWriter` for both live and file-based `-3js`, so the HUD works in playback too. Stats accumulate every step in `accumulateContractilityStats()`; `[STATS] contractility …` also prints the same to stdout at frame cadence.

**Tuning panel** (`#contractilityParams`, right side, live mode only): a curated set of runtime-mutable knobs grouped Bind / Cross-bridge / Detachment, driven by the standard `queryParams`/`setParam` WebSocket plumbing. Knobs: `myoColTol` (bind capture radius), `myoMotorAlignWithFilTolerance`; `myoSpring` (cross-bridge stiffness — promoted from a hardcoded `MyoFilLink` constant), `myosinStallForce`, `myosinBreakForce`; `kOff`, `xCatch`, `xSlip`, `alphaCatch`, `alphaSlip` (Guo–Guilford catch-slip). All are `.setMutableAtRuntime()`. **On CPU runs every knob is live each step** (the recommended tuning mode — the assay is CPU-fast). On `-gpu`, force/release/stiffness are live (packed `EVERY_EXECUTION`); the **bind** knobs are baked into `gridParams` at `FIRST_EXECUTION` so they take effect on restart (a `gridParams` `EVERY_EXECUTION` rewire is the deferred follow-up). Note the knobs interact: e.g. raising `myoSpring` without raising `myosinBreakForce` makes heads hit the break threshold at lower strain and snap off, *reducing* net tension. Launch for tuning: `BoxOfActin -contractility -3jsLive 8081` (CPU), open `…?live=8081`.
