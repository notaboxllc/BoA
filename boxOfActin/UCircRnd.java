...
public class UCircRnd {
	...
	public void newValue (double deltaT,Thing thing) {
		rsq = 0;
		while ((rsq >= 1.0) | (rsq == 0)) {	// while length of vector isn't in the unit circle do..
			v1 = 2*thing.myPRNG.nextDouble()-1;		// one uniform number in the square from -1 to 1...
			v2 = 2*thing.myPRNG.nextDouble()-1;		// ... and another one.
			rsq = v1*v1 + v2*v2;			// length of the vector.
		}
		facterm = -4*deltaT*Math.log(rsq)/rsq;
	}
