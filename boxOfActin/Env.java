...
public class Env {
	...
	static final Parameter dtsActinPushMax = new Parameter("dtsActinPushMax"," DTS actin->membrane push cap (stall force)", 5.0e-12, "N")
		.setMutableAtRuntime()
		.setDescription("Ceiling (N) on the steric force a filament (e.g. a growing Arp2/3 daughter) exerts OUTWARD on the cortex in the protrusion regime (tip/body inside the surface, pressing out). Set to the actin polymerization STALL force (~5 pN) so a daughter pushes the membrane only as hard as a real filament can before its growth halts -- the Mogilner-Oster Brownian ratchet (which throttles growth as the tip-membrane gap closes) does the rest. This keeps daughters from shoving the cortex with the stiff containment-scale force and convoluting it. A sample that has actually CROSSED outside still gets the stiff hard-recovery (dtsStericRecover, capped at 2e-10) for guaranteed containment. Default 5e-12 (~5 pN).");
	static final Parameter dtsForminStandoff = new Parameter("dtsForminStandoff"," DTS formin barbed-end standoff off the membrane", 0.05, " microns")
		.setMutableAtRuntime()
		.setDescription("Distance (microns) a formin (complex) holds the barbed end INSIDE the cortex, measured inward along the vertex's outward radial. The formin-membrane tether targets this standoff point, NOT the vertex itself -- so the tether's rest position agrees with where the steric collision keeps the filament (just inside the surface). With the two in agreement there is no battle between tether (pulling the barbed end to the surface) and collision (pushing it inside), so the bond exerts ~zero net reaction on the vertex and the membrane stays smooth instead of cratering at each attachment point. Should be >= filTipRadiusForCollision. Default 0.05.");
	static final Parameter dtsAnchorForceMax = new Parameter("dtsAnchorForceMax"," DTS membrane-actin anchor force cap", 1.0e-10, "N")
		.setMutableAtRuntime()
		.setDescription("Ceiling (N) on the formin-membrane anchor spring force in Membrane.collideActin, per end. This anchor is the SOLE bond between a held filament and its membrane vertex (the stiff legacy fracMove node tether is skipped for MembraneVertex bonds, so it doesn't rigidly drag/convolute the cortex). Default 1e-10 (~0.1 pN). Raise to hold filaments more firmly; lower for an even softer bond. The cap also guards against a NaN-inducing kick from a large transient offset.");
	...
	static private double forminBarbedBrownianFactor_init = 0.05; // 1/20: a formin holds the barbed end at the DTS cortex
	static final Parameter forminBarbedBrownianFactor = new Parameter("forminBarbedBrownianFactor"," Formin-Barbed-Held Brownian Factor", forminBarbedBrownianFactor_init, "").setMutableAtRuntime().setDescription("Brownian-force multiplier for a formin mother whose BARBED end is held at the DTS membrane cortex (Stage-3e dtsForminGrowOut, nodeAtEnd2). Such a filament is processively bound to the formin at the membrane, not freely diffusing; a full free-filament thermal kick makes it twist and convolute the membrane through the formin-membrane anchor spring. Scaled down by this factor (default 0.05 = 1/20). 1.0 = treat as free. Analogous to arpHeldBrownianFactor for the pointed-end (nodeAtEnd1) hold.");
	static private double arpDaughterBrownianFactor_init = 1.0; // 1.0 = off (no extra daughter damping)
	static final Parameter arpDaughterBrownianFactor = new Parameter("arpDaughterBrownianFactor"," Arp2/3 Daughter Brownian Factor", arpDaughterBrownianFactor_init, "").setMutableAtRuntime().setDescription("Extra Brownian-force multiplier applied to EVERY Arp2/3 daughter (motherFil != null), on top of the mother's inherited damping and the drag-ratio share. A daughter in a branched network is constrained by its junction and neighbours, not freely diffusing; heavy damping (e.g. 0.05) keeps daughters from thrashing and spinning the network. 1.0 = off. Note: the daughter Brownian now also respects the global BTransCoeff/BRotCoeff, so BTransCoeff=BRotCoeff=0 turns Brownian fully off for mothers AND daughters (e.g. for a junction-only spin test).");
