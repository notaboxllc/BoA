...
//	 file, file writer, and print writer for apparent diffusivity file
    ...
    */
	...
	public static void saveView (Transform3D viewTrans) {
		Matrix4d m1 = new Matrix4d();
		Dimension screenSize = new Dimension(Env.frameWidth.getIntValue(),Env.frameHeight.getIntValue());
		viewTrans.get(m1);
		try {
			File viewFile = new File(getFileToSave("Save view to...", null));
			FileOutputStream viewFOS = new FileOutputStream(viewFile);
			ObjectOutputStream viewOOS = new ObjectOutputStream(viewFOS);
			viewOOS.writeObject(m1);
			viewOOS.writeObject(screenSize);
			talkln ("View saved to " + viewFile.getAbsolutePath());
		} catch (IOException ioe) { talkln ("Exception saving view: " + ioe); }
	}
	
	public static Transform3D readView (String viewFileStr) {
		...
		try {
			if (viewFileStr == null) {
				viewFile = new File(getFileToLoad("Load view from...",null));
			} else {
				viewFile = new File(viewFileStr);
			}
			FileInputStream viewFIS = new FileInputStream(viewFile);
			ObjectInputStream viewOIS = new ObjectInputStream(viewFIS);
			m1 = (Matrix4d)viewOIS.readObject();
			screenSize = (Dimension)viewOIS.readObject();
			view.set(m1);
			Env.frameWidth.setValue(screenSize.width);
			Env.frameHeight.setValue(screenSize.height);
			if (!BoxOfActin.boaGraphics.canvasSizeCurrent()) { BoxOfActin.boaGraphics.makeCanvasSizeCurrent(); }

		} catch (Exception ioe) { talkln ("Exception reading view: " + ioe); }
		return view;
	}
...
/*	public static void loadParamConfig (File paramFile, boolean restartNeeded) {
	...
	*/
	...
	public static void takeQuickPicture (String fileName) { 
		Thing curT;
		Monomer mon;
		FilSegment fil;
		MyoMotor myoMot;
		MyoLever myoLev;
		MyoRod myoRod;
		MyoMiniFilament myoMiniFil;
		ProteinNode plas;
		Chamber chamber;
		Bug bug;
		int noTs = Thing.thingCt;
		try {
			FileOutputStream fos = new FileOutputStream (fileName);
			DataOutputStream ds = new DataOutputStream (fos);
			// write general data before Things...
			// time
			ds.writeInt(FileOps.TIME);
			ds.writeFloat((float)Env.simulationTime);
			// actin concentration
			ds.writeInt(FileOps.ACTIN_CONCENTRATION);
			ds.writeFloat(Env.actinConc.getFloatValue());
			// Plasmid number
			ds.writeInt(FileOps.NUM_PLASMIDS);
			ds.writeInt(ProteinNode.nodeCt);
			// Filament number
			ds.writeInt(FileOps.NUM_FILS);
			ds.writeInt(FilSegment.getNumberOfFilaments());
			// Filament segment number
			ds.writeInt(FileOps.NUM_FILSEGS);
			ds.writeInt(FilSegment.filSegmentCt);
			// Monomer number
			ds.writeInt(FileOps.NUM_MONS);
			ds.writeInt(FilSegment.monomerSum());
			// FilLinks
			ds.writeInt(FileOps.NUM_FILLINKS);
			ds.writeInt(FilLink.filLinkCt);
			// Myosins
			ds.writeInt(FileOps.NUM_MYOSINS);
			ds.writeInt(Myosin.myoCt);
			// Myosin Dimers
			ds.writeInt(FileOps.NUM_MYOSINDIMERS);
			ds.writeInt(MyosinDimer.myoDimerCt);
			// Myosin Minifilaments
			ds.writeInt(FileOps.NUM_MYOMINIFILS);
			ds.writeInt(MyoMiniFilament.myoMiniFilCt);
			// Number of Things
			ds.writeInt(FileOps.NUM_THINGS);
			ds.writeInt(noTs);
			for (int i = 0; i<noTs; i++) {
				curT = Thing.theThings[i];
				if (curT instanceof FilSegment) {
					fil = (FilSegment) curT;
					ds.writeByte((byte)FILSEGMENT);	// identify as a filament
					Pt3D.writePt3D(ds,fil.coord);
					Pt3D.writePt3D(ds,fil.uVec);
					ds.writeInt(fil.monomerCt);
					if (fil.filAtEnd2) { ds.writeInt(fil.end2Fil.myThingNumber); } else { ds.writeInt(-1); }  // myThingNumber of filsegment at end2
					if (fil.nodeAtEnd1) {  // myThingNumber of linked plasmids
						ds.writeInt(fil.end1Node.myThingNumber); 
						Pt3D.writePt3D(ds, fil.end1PAttachPt);
					} else { ds.writeInt(-1); }	
					if (fil.nodeAtEnd2) { 
						ds.writeInt(fil.end2Node.myThingNumber); 
						Pt3D.writePt3D(ds, fil.end2PAttachPt);
					} else { ds.writeInt(-1); }
					mon = fil.minusMon;
					int monWriteCt = 0;
					ds.writeBoolean(!Env.noMonomersSimd.isActive());
					while (!Env.noMonomersSimd.isActive() & mon != Monomer.plusGhost) {
						if (mon.monVec3D == null) {
							talk ("Writing null mon position in FileOps.takeQuickPicture()");
							talk ("  Monomer = " + mon);
							talk ("  Monomer.frontMon = " + mon.frontMon);
							talkln ("MonomerCt = " + fil.monomerCt + ", but monWriteCt = " + monWriteCt);
							for (int k = monWriteCt; k < fil.monomerCt; k++) {
								Pt3D.writeNullVec3d(ds);
								ds.writeInt(-666);
								ds.writeFloat(0);
							...
							Pt3D.writeVec3d(ds,mon.monVec3D);
							monWriteCt++;
						}
						ds.writeInt(mon.getState());
						ds.writeFloat((float)mon.xRotAng);
						ds.writeBoolean(mon.cofilinOn);
						mon = mon.frontMon;
					}
				} else if (curT instanceof MyoMotor) {
					myoMot = (MyoMotor) curT;
					ds.writeByte((byte)MYOMOTOR);
					Pt3D.writePt3D(ds, myoMot.coord);
					Pt3D.writePt3D(ds, myoMot.uVec);
					ds.writeFloat((float)myoMot.getDim());
					ds.writeByte(myoMot.nucleotideState);
				} else if (curT instanceof MyoLever) {
					myoLev = (MyoLever) curT;
					ds.writeByte((byte)MYOLEVER);
					Pt3D.writePt3D(ds, myoLev.coord);
					Pt3D.writePt3D(ds, myoLev.uVec);
					ds.writeFloat((float)myoLev.getDim());
				} else if (curT instanceof MyoRod) {
					myoRod = (MyoRod) curT;
					ds.writeByte((byte)MYOROD);
					Pt3D.writePt3D(ds, myoRod.coord);
					Pt3D.writePt3D(ds, myoRod.uVec);
					ds.writeFloat((float)myoRod.getDim());
					ds.writeBoolean(myoRod.rodInvisible);
				} else if (curT instanceof MyoMiniFilament) {
					myoMiniFil = (MyoMiniFilament) curT;
					ds.writeByte((byte)MYOMINIFIL);
					Pt3D.writePt3D(ds, myoMiniFil.coord);
					Pt3D.writePt3D(ds, myoMiniFil.uVec);
					ds.writeFloat((float)myoMiniFil.getLength());
					ds.writeFloat((float)myoMiniFil.getRadius());
				} else if (curT instanceof ProteinNode) {
					plas = (ProteinNode) curT;
					ds.writeByte((byte)PLASMID);	// identify as a node
					Pt3D.writePt3D(ds,plas.coord);
					ds.writeFloat((float)plas.getRadius());
				} else if (curT instanceof Chamber) {
					chamber = (Chamber) curT;
					ds.writeByte((byte)BOX);
					ds.writeFloat((float)chamber.getXDim());
					ds.writeFloat((float)chamber.getYDim());
					ds.writeFloat((float)chamber.getZDim());
				} else if (curT instanceof Bug) {
					bug = (Bug) curT;
					ds.writeByte((byte)BUG);
					ds.writeFloat((float)bug.getLength());
					ds.writeFloat((float)bug.getRadius());
				}	
			} 
			
			writeFilLinks (ds);
				
		} catch (Exception ioe) { talkln ("some sort of error writing qk file"); }
	}
	
	public static void writeFilLinks (DataOutputStream ds) {
		// write out all the FilLink info
		try {
			for (int i=0;i<FilLink.filLinkCt;i++) {
				ds.writeByte((byte)FILLINK);
				if (FilLink.filLinks[i] != null && FilLink.filLinks[i].pt1 != null && FilLink.filLinks[i].pt2 != null) {	// check if link's gone to null
					Pt3D.writePt3D(ds,FilLink.filLinks[i].pt1);
					Pt3D.writePt3D(ds,FilLink.filLinks[i].pt2);
				} else {
					Pt3D.writePt3D(ds, Env.farfarAway);
					Pt3D.writePt3D(ds, Env.farfarAway);
				}
			}
		} catch (Exception ioe) { talkln ("error in writeFilLinks"); }
	}
	
	public static void writeMyosins (DataOutputStream ds) {
		// write out all the Myosin info
		try {
			Myosin curMyo;
			for (int i=0;i<Myosin.myoCt;i++) {
				ds.writeByte((byte)MYOSIN);
				if (Myosin.theMyosins[i] != null) {	
					curMyo = Myosin.theMyosins[i];
					// motor info
					ds.writeByte(curMyo.myoMotor.nucleotideState);
					ds.writeFloat(Env.myoMotorLength.getFloatValue());
					Pt3D.writePt3D(ds,curMyo.myoMotor.coord);
					Pt3D.writePt3D(ds,curMyo.myoMotor.uVec);
					// lever-arm info
					ds.writeFloat(Env.myoLeverLength.getFloatValue());
					Pt3D.writePt3D(ds,curMyo.myoLever.coord);
					Pt3D.writePt3D(ds,curMyo.myoLever.uVec);
					// rod info
					ds.writeFloat(Env.myoRodLength.getFloatValue());
					Pt3D.writePt3D(ds,curMyo.myoRod.coord);
					Pt3D.writePt3D(ds,curMyo.myoRod.uVec);
				} else {
					Pt3D.writePt3D(ds, Env.farfarAway);
					Pt3D.writePt3D(ds, Env.farfarAway);
				}
			}
		} catch (Exception ioe) { talkln ("error in writeMyosins"); }
	}
	
	synchronized static void loadQuickPicture (String curFile) {
		Env.fromQKFile = true;
		int id = FileOps.NOTHING;
		int noTs = 0;	// number of Things
		int noFLs = 0;	// number of FilLinks
		int noPlas = 0;	// number of Plasmids
		int noMyos = 0; // number of Myosins
		int noMyoDs = 0; // number of Myosin Dimers
		int noMiniFils = 0; // number of Myosin Minifilaments
		int curType;
		int curMonCt;
		int plasOn = 0;	// 1 if plasmid attached, 0 if not
		Pt3D loc = new Pt3D();
		Pt3D monLoc = new Pt3D();
		Pt3D ang = new Pt3D();
		Pt3D wastedPt3D = new Pt3D();
		float dim;
		float rad;
		boolean invis;
		// thing counters
		...
		try {
			...
			while (id != FileOps.NUM_THINGS) {
				id = ds.readInt();
				switch (id) {
				case FileOps.TIME:
					Env.simulationTime = ds.readFloat();
					break;
				case FileOps.ACTIN_CONCENTRATION:
					Env.actinConc.setValue(ds.readFloat());
					break;
				case FileOps.NUM_PLASMIDS:
					noPlas = ds.readInt();
					break;
				case FileOps.NUM_FILS:
					FilSegment.filamentRenderCt = ds.readInt();
					break;
				case FileOps.NUM_FILSEGS:
					FilSegment.filSegRenderCt = ds.readInt();
					break;
				case FileOps.NUM_MONS:
					Monomer.monRenderCt = ds.readInt();
					if (Env.filRenderOff) { Monomer.monCt = 0; }
					break;
				case FileOps.NUM_FILLINKS:
					noFLs = ds.readInt();
					FilLink.filLinkRenderCt = noFLs;
					break;
				case FileOps.NUM_MYOSINS:
					noMyos = ds.readInt();
					break;
				case FileOps.NUM_MYOSINDIMERS:
					noMyoDs = ds.readInt();
					break;
				case FileOps.NUM_MYOMINIFILS:
					noMiniFils = ds.readInt();
					break;
				case FileOps.NUM_THINGS:
					noTs = ds.readInt();	// number of things to read in following loop
					break;
				}
			}
			for (int i=0;i<noTs;i++) {
				curType = (int)ds.readByte();
				switch (curType) {
				case FILSEGMENT:
					double locShift = 0;
					loc.readPt3D(ds);
					ang.readPt3D(ds);
					if (Env.orderedCentered) { 
						locShift = loc.x;
						loc.x = 0; 
					}
					curMonCt = ds.readInt();
					ds.readInt();	// read in myThingNumber of end2 filsegment (or -1 if no link), unused here
					plasOn = ds.readInt();	// plasmid at end1
					if (plasOn != -1 ) { wastedPt3D.readPt3D(ds); } // read in attachPt.. unused
					plasOn = ds.readInt();	// plasmid at end2 
					if (plasOn != -1 ) { wastedPt3D.readPt3D(ds); } // read in attachPt.. unused
					boolean monsWereWritten = ds.readBoolean();
					if (monsWereWritten) {
						int monState;
						float xRotAng;
						boolean cofilinOn;
						for (int m=0;m<curMonCt;m++) {
							monLoc.readPt3D(ds);
							monState = ds.readInt();
							xRotAng = ds.readFloat();
							cofilinOn = ds.readBoolean();
							if (!Env.noMonomersSimd.isActive()) {
								if (Monomer.monCt > monSum) {
									if (Env.orderedCentered) { monLoc.x += -locShift; }
									Monomer.theMonomers[monSum].setFromQKInfo(monLoc,monState,xRotAng,cofilinOn);
								} else {
									Monomer.theMonomers[Monomer.monCt] = new Monomer();
									if (Env.orderedCentered) { monLoc.x += -locShift; }
									Monomer.theMonomers[Monomer.monCt].setFromQKInfo(monLoc,monState,xRotAng,cofilinOn);
									Monomer.monCt++;
								}
							}
							monSum++;
					...
					if (Env.noMonomersSimd.isActive()) {
						if (FilSegment.filSegmentCt > filSegSum) {
							FilSegment.theFilSegments[filSegSum].setFromQKInfo(loc,ang,curMonCt);
						} else {
							new FilSegment(loc,ang,-1,curMonCt,true);
						}
						filSegSum++;
					} 
					break;
				case MYOMOTOR:
					...
					ang.readPt3D(ds);
					dim = ds.readFloat();
					byte state = ds.readByte();
					if (Myosin.myoCt > myoSum) {
						Myosin.theMyosins[myoSum].setMotor(loc,ang,dim,state);
					} else {
						new Myosin();
						Myosin.theMyosins[myoSum].setMotor(loc,ang,dim,state);
					}
				...
				case MYOLEVER:
					...
					ang.readPt3D(ds);
					dim = ds.readFloat();
					Myosin.theMyosins[myoSum].setLever(loc,ang,dim);
					break;
				case MYOROD:
					loc.readPt3D(ds);
					ang.readPt3D(ds);
					dim = ds.readFloat();
					invis = ds.readBoolean();
					Myosin.theMyosins[myoSum].setRod(loc,ang,dim,invis);
					myoSum++;
					break;
				case MYOMINIFIL:
					loc.readPt3D(ds);
					...
					dim = ds.readFloat();
					rad = ds.readFloat();
					if (MyoMiniFilament.myoMiniFilCt > myoMiniFilSum) {
						MyoMiniFilament.myoMiniFils[myoMiniFilSum].set(loc,ang,dim,rad);
					} else {
						new MyoMiniFilament(loc,ang,dim,rad);
					}
					myoMiniFilSum++;
					break;
				case PLASMID:
					loc.readPt3D(ds);
					double plasRad = ds.readFloat();
					if (ProteinNode.nodeCt > plasSum) {
						ProteinNode.theNodes[plasSum].setFromQK(plasRad, loc);
					} else {
						new ProteinNode(loc,true);
					}
					plasSum++;
					if (Env.filRenderOff & (plasSum == noPlas)) { return; }
					break;
				case BOX:
					if (Thing.theBox == null) {
						Env.boxXDim.setValue((double)ds.readFloat());
						Env.boxYDim.setValue((double)ds.readFloat());
						Env.boxZDim.setValue((double)ds.readFloat());
						Chamber.makeABox();
					} else {
						ds.readFloat(); // x
						ds.readFloat(); // y
				...
				case BUG:
					if (Thing.theBox == null) {
						Env.bugLength.setValue((double)ds.readFloat());
						Env.bugRadius.setValue((double)ds.readFloat());
			...
			if (!Env.noMonomersSimd.isActive() && monSum != Monomer.monRenderCt) { talkln("monSum != Monomer.monRenderCt.... check that out"); }
			
			// FilLinks... not a subclass of Thing
			if (Env.showXLinks.isActive()) {
				Pt3D filLinkPt1 = new Pt3D();
				Pt3D filLinkPt2 = new Pt3D();
				for (int i=0;i<noFLs;i++) {
					curType = (int)ds.readByte();
					if (curType != FILLINK) { talkln ("Thought we were reading FILLINK but we're not"); }
					filLinkPt1 = Pt3D.ReadPt3D(ds);
					filLinkPt2 = Pt3D.ReadPt3D(ds);
					if (i<FilLink.filLinkCt) {
						//FilLink.filLinks[i].setPtsFromQKFile(filLinkPt1,filLinkPt2);
					} else {
						//new FilLink (filLinkPt1,filLinkPt2);
					}
				}
				// effectively remove graphics from unused FilLinks
				for (int i=noFLs;i<FilLink.filLinkCt;i++) { FilLink.filLinks[i].setPtsFromQKFile(Env.farfarAway,Env.farfarAway); }
			}
			
		} catch (IOException ioe) { talkln ("some sort of error reading .qk file " + curFile); }
	}
	
	synchronized static void loadQuickState (String curFile) {
		Env.loadingQKFile = true;
		BoxOfActin_Graphics.clearForRun();
		FilLink.filLinkCt = 0;
		int id = FileOps.NOTHING;
		int noTs = 0;
		int curType;
		...
		int monState;
		int filSum = 0;
		int end2ThingNumber = -1;
		Pt3D loc = new Pt3D();
		Pt3D vec = new Pt3D();
		FilSegment fil;
		Monomer curMon = new Monomer();
		try {
			FileInputStream fis = new FileInputStream (curFile);
			DataInputStream ds = new DataInputStream (fis);
			while (id != FileOps.NUM_THINGS) {
				...
				case FileOps.NUM_FILS:
					...
					break;
				case FileOps.NUM_FILSEGS:
					ds.readInt(); // unused... value set when filsegs created
					break;
				case FileOps.NUM_MONS:
					ds.readInt();	//unused
					break;
			...
			for (int i=0;i<noTs;i++) {
				...
				case FILSEGMENT:
					...
					vec.readPt3D(ds);
					curMonCt = ds.readInt();
					fil = new FilSegment(loc,vec,-1,curMonCt,true);
					fil.end2LinkThingNumber = ds.readInt();
					
					// plasmids
					fil.end1NodeThingNumber = ds.readInt();	// plasmid at end1
					if (fil.end1NodeThingNumber != -1) { fil.end1PAttachPt.readPt3D(ds); }
					fil.end2NodeThingNumber = ds.readInt();	// plasmid at end2 
					if (fil.end2NodeThingNumber != -1) { fil.end2PAttachPt.readPt3D(ds); }
					// set monomer states
					curMon = fil.minusMon;
					for (int m=0;m<curMonCt;m++) {
						loc.readPt3D(ds);	// unused but must read
						monState = ds.readInt();
						curMon.setState(monState);
						curMon = curMon.frontMon;
					}
					break;
				case PLASMID:
					loc.readPt3D(ds);
					Env.nodeRadius.setValue((double)ds.readFloat());
					new ProteinNode(loc,true);
					break;
				case BOX:
					Env.boxXDim.setValue((double)ds.readFloat());
			...
			// set FilSegment links to other FilSegments and Plasmids
			FilSegment fil1, fil2;
			ProteinNode plas;
			for (int i=0;i<FilSegment.filSegmentCt;i++) {
				fil1 = FilSegment.theFilSegments[i];
				if (fil1.end2LinkThingNumber != -1) {
					if (Thing.theThings[fil1.end2LinkThingNumber] instanceof FilSegment) {
						fil2 = (FilSegment)Thing.theThings[fil1.end2LinkThingNumber];
						FilSegment.link(fil1, fil2);
					} else {
						System.out.println ("Error in linking filsegments after QK state read");
					}
				}
				
				if (fil1.end1NodeThingNumber != -1) {
					if (Thing.theThings[fil1.end1NodeThingNumber] instanceof ProteinNode) {
						plas = (ProteinNode)Thing.theThings[fil1.end1NodeThingNumber];
						FilSegment.linkEnd1Node(fil1,plas);
					} else {
						System.out.println ("Error in linking filsegment and plasmid after QK state read");
					}
				}
				
				if (fil1.end2NodeThingNumber != -1) {
					if (Thing.theThings[fil1.end2NodeThingNumber] instanceof ProteinNode) {
						plas = (ProteinNode)Thing.theThings[fil1.end2NodeThingNumber];
						FilSegment.linkEnd2Node(fil1,plas);
					} else {
						System.out.println ("Error in linking filsegment and plasmid after QK state read");
					}
				}
		...
		} catch (IOException ioe) { talkln ("some sort of error loading .qk state " + curFile); }
		Env.loadingQKFile = false;
		//System.out.println ("Done loading quick state");
		BoxOfActin.boaGraphics.updateBugScene();
		//System.out.println ("Done updating bug scene");
	}
