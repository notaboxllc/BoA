...
/**
 * DeflectionTunerV24 — V23 + noise-aware Jacobian gate + physics-aware convergence tolerance.
 *
 * Changes from V23:
 *  Fix 1: Broyden update skipped when either dp component is below threshold (prevents
 *          dividing noisy deltaS by a tiny deltaParam when we're close to target).
 *  Fix 2: CONV_TOL_UM is now relative to target deflection (0.5% of target, floor 1e-5 µm),
 *          so the algorithm cannot be asked to resolve below the measurement noise floor.
 */
class DeflectionTunerV24 {
    ...
    String resultSummary() {
        return String.format(
            "[V24] %s  fracR=%.4f  fracMoveTorq=%.4f  fracMove=%.4f  iters=%d  drops=%d  frames=%d",
    ...
    private double convTolUm;  // physics-aware tolerance, computed in start()
    ...
    void start(double fm, double frUnused, double fmtUnused, double targetMicrons, double tauUnused) {
        ...
        System.out.printf("[V24] armed: fr=%.4f  fmt=%.4f  fracMove=%.4f  target=%.6f µm%n",
            fr, fmt, fracMove, target);
        System.out.printf("[V24]   bounds: FR=[%.2f,%.2f]  FMT=[%.2f,%.2f]%n",
            FR_LO, FR_HI, FMT_LO, FMT_HI);
        System.out.printf("[V24]   CONV_TOL=%.2e µm  (rel=%.3f × target=%.6f)%n",
            convTolUm, CONV_TOL_REL_TARGET, target);
        System.out.printf("[V24]   SETTLE_WIN=%d  SETTLE_MIN=%d  SETTLE_MAX=%d%n",
            SETTLE_WIN, SETTLE_MIN_FRAMES, SETTLE_MAX_FRAMES);
        System.out.printf("[V24]   PERT_FR=%.2f  PERT_FMT=%.2f  DAMPING=%.2f  MAX_ITER=%d  MAX_FM_DROPS=%d%n",
            PERT_FR, PERT_FMT, DAMPING, MAX_ITERATIONS, MAX_FRACMOVE_DROPS);
        System.out.printf("[V24]   J_THRESH_FR=%.3f  J_THRESH_FMT=%.3f%n",
            JACOBIAN_UPDATE_DP_THRESH_FR, JACOBIAN_UPDATE_DP_THRESH_FMT);
        System.out.printf("[V24] settling seed-1  fr=%.4f  fmt=%.4f%n", fr, fmt);
    }
    ...
    private ParamTriple onSettled(double s) {
        switch (phase) {
            case SEEDING_1:
                ...
                System.out.printf("[V24] settling seed-2  fr=%.4f  fmt=%.4f%n", fr, fmt);
            ...
            case SEEDING_2:
                ...
                System.out.printf("[V24] settling seed-3  fr=%.4f  fmt=%.4f%n", fr, fmt);
                return new ParamTriple(fracMove, fr, fmt);
            ...
            case SEEDING_3: {
                ...
                System.out.printf("[V24] Jacobian seeded: J=[%.4e, %.4e]  s1=%.6f  s2=%.6f  s3=%.6f%n",
                    J[0], J[1], s1, s2, s);
    ...
    private ParamTriple onNewtonSettled(double s) {
        ...
        if (dpdp > 1e-20) {
            // Fix 1: gate the Broyden update — skip if either dp component is below noise threshold.
            // A small dp means the Newton step was tiny (we're close to target); the resulting
            // deltaS is dominated by measurement noise, and dividing by tiny dp corrupts the Jacobian.
            if (Math.abs(dp0) < JACOBIAN_UPDATE_DP_THRESH_FR
                    || Math.abs(dp1) < JACOBIAN_UPDATE_DP_THRESH_FMT) {
                System.out.printf("[V24:JACOBIAN_FROZEN] dp_fr=%.4e  dp_fmt=%.4e  reason=below_dp_threshold%n",
                    dp0, dp1);
                // Keep J unchanged; pPrev/sPrev updated below.
            } else {
                double ds           = s - sPrev;
                ...
                System.out.printf("[V24:JACOBIAN] J=[%.4e, %.4e]  ds=%.4e  pred_ds=%.4e  quality=%.3f%n",
        ...
        if (stagnantCount >= 3) {
            System.out.printf("[V24:JACOBIAN_RESET] stalled=true  re-seeding from (fr=%.4f  fmt=%.4f)%n", fr, fmt);
            stagnantCount = 0;
            ...
            changeParams(fr2, fmt2);
            phase = Phase.SEEDING_2;
            System.out.printf("[V24] settling seed-2 (re-seed)  fr=%.4f  fmt=%.4f%n", fr, fmt);
            return new ParamTriple(fracMove, fr, fmt);
        }
    ...
    private ParamTriple applyNewtonStep(double s) {
        ...
        if (Math.abs(r) < convTolUm) {
            ...
            System.out.printf(
                "[V24:CONVERGED] iterations=%d  fracMove_drops=%d  total_frames=%d  final=(fr=%.4f  fmt=%.4f  fracMove=%.4f)  s=%.6f  tol_used=%s%n",
                newtonIter, fracMoveDrops, totalFrames, fr, fmt, fracMove, s, tolUsed);
        ...
        if (newtonIter > MAX_ITERATIONS) {
            System.out.printf("[V24:FAILED] reason=iteration_limit  iterations=%d  fracMove=%.4f%n",
                newtonIter, fracMove);
            phase = Phase.FAILED;
            return null;
        }
        ...
        if (JJT < 1e-30) {
            System.out.printf("[V24:FAILED] reason=jacobian_zero  fracMove=%.4f%n", fracMove);
            phase = Phase.FAILED;
            return null;
        }
        ...
        if (Math.abs(dp0) > maxDp0) {
            System.out.printf("[V24:CLAMPED] param=fr  requested=%.6f  applied=%.6f%n",
                dp0, Math.signum(dp0) * maxDp0);
            dp0 = Math.signum(dp0) * maxDp0;
        }
        if (Math.abs(dp1) > maxDp1) {
            System.out.printf("[V24:CLAMPED] param=fmt  requested=%.6f  applied=%.6f%n",
        ...
        if (newFr < FR_LO) {
            System.out.printf("[V24:BOUND] param=fr  hit=lo  value=%.4f%n", newFr);
            newFr = FR_LO;   atBound = true;
        } else if (newFr > FR_HI) {
            System.out.printf("[V24:BOUND] param=fr  hit=hi  value=%.4f%n", newFr);
            newFr = FR_HI;   atBound = true;
        }
        if (newFmt < FMT_LO) {
            System.out.printf("[V24:BOUND] param=fmt  hit=lo  value=%.4f%n", newFmt);
            newFmt = FMT_LO; atBound = true;
        } else if (newFmt > FMT_HI) {
            System.out.printf("[V24:BOUND] param=fmt  hit=hi  value=%.4f%n", newFmt);
            newFmt = FMT_HI; atBound = true;
        }

        iterationsAtCorner = atBound ? iterationsAtCorner + 1 : 0;

        System.out.printf("[V24:NEWTON#%d] r=%.4e  fracMove=%.4f  fr: %.4f→%.4f  fmt: %.4f→%.4f%n",
    ...
    private ParamTriple handleInfeasible(double r, double frCorner, double fmtCorner) {
        if (r > 0) {
            System.out.printf(
                "[V24:INFEASIBLE_2D] fracMove=%.4f  corner=(fr=%.4f, fmt=%.4f)  residual=%.4e  direction=stiff%n",
                fracMove, frCorner, fmtCorner, r);
            System.out.printf(
                "[V24:FAILED] reason=needs_stiffer  fracMove_at_ceiling=0.5  raise_above_0.5_for_solution=true%n");
            phase = Phase.FAILED;
            return null;
        }

        System.out.printf(
            "[V24:INFEASIBLE_2D] fracMove=%.4f  corner=(fr=%.4f, fmt=%.4f)  residual=%.4e  direction=soft%n",
            fracMove, frCorner, fmtCorner, r);

        if (fracMoveDrops >= MAX_FRACMOVE_DROPS) {
            System.out.printf("[V24:FAILED] reason=fracmove_iteration_limit  drops=%d%n", fracMoveDrops);
            phase = Phase.FAILED;
            return null;
        }
        ...
        fracMoveDrops++;
        System.out.printf("[V24:FRACMOVE_DROP] fracMove: %.2f → %.2f  restarting 2D from (fr=%.4f  fmt=%.4f)%n",
            oldFm, fracMove, frCorner, fmtCorner);

        initTwoDimSearch(frCorner, fmtCorner);
        System.out.printf("[V24] settling seed-1  fr=%.4f  fmt=%.4f  fracMove=%.4f%n", fr, fmt, fracMove);
        return new ParamTriple(fracMove, fr, fmt);
    }
    ...
    private void logSettled(double s, boolean timedOut) {
        ...
        if (timedOut) {
            System.out.printf(
                "[V24:SETTLE_TIMEOUT] fr=%.4f  fmt=%.4f  fracMove=%.4f  frames=%d  std/mean=%.4f%n",
                fr, fmt, fracMove, framesAtParams, relStd);
        }
        System.out.printf(
            "[V24:SETTLED] params=(fr=%.4f  fmt=%.4f  fracMove=%.4f)  s=%.6f  frames=%d  std/mean=%.4f%n",
