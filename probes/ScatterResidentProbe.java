/*
 * ScatterResidentProbe — TornadoVM 4.0.1-dev feasibility probe.
 *
 ...
 */
public class ScatterResidentProbe {
    ...
    public static void main(String[] args) {
        ...
        uk.ac.manchester.tornado.api.TaskGraph tg =
            new uk.ac.manchester.tornado.api.TaskGraph("probe")
                ...
                .task("scatter",
                      ScatterResidentProbe::scatterKernel,
                      pose, deltaVal, deltaIdx, count)
                .task("integrate",
                      ScatterResidentProbe::integrateKernel,
                      pose)
                .transferToHost(
        ...
        //   pose[5]: count=1 only on s==50. The scatter writes once; integrate
        //            runs on every execute. The 200 executes split around step 50
        //            into: 50 executes before, 1 at step 50, 149 executes after.
        //            If scatter runs BEFORE integrate on step 50:
        //              pre-50: 50 (50 integrates)
        //              step 50: scatter→42, integrate→43
        //              post-50: +149 integrates → 192.0f.
        //            If scatter runs AFTER integrate on step 50:
        //              pre-50 + step 50's integrate: 51
        //              scatter: 42
        //              post-50: +149 → 191.0f.
        //            (no scatter applied / silently dropped: 200.0f.)
        float expectedResident          = 200.0f;
        float expectedReset             =   1.0f;
        float expectedScatterBeforeIntg = 192.0f;
        float expectedScatterAfterIntg  = 191.0f;
        ...
        System.out.printf("  expected %.1f (scatter BEFORE integrate on step 50; declaration honored)%n", expectedScatterBeforeIntg);
        System.out.printf("  expected %.1f (scatter AFTER integrate on step 50; declaration reversed)%n", expectedScatterAfterIntg);
